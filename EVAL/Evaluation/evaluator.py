...
class Eval_thread():
    ...
    def run(self):
        ...
        # 计算Dice和mIoU
        dice, miou = self.Eval_dice_mIoU()
        Res['Dice'] = dice
        Res['mIoU'] = miou

        self.LOG(
            '{} ({}): {:.4f} mae || {:.4f} max-fm || {:.4f} mean-fm || {:.4f} max-Emeasure || {:.4f} mean-Emeasure || {:.4f} S-measure || {:.4f} AP || {:.4f} AUC || {:.4f} Dice || {:.4f} mIoU.\n'
            .format(self.dataset, self.method, mae, max_f, mean_f, max_e,
                    mean_e, s, avg_p, auc, dice, miou))
        return '[cost:{:.4f}s] {} ({}): {:.4f} mae || {:.4f} max-fm || {:.4f} mean-fm || {:.4f} max-Emeasure || {:.4f} mean-Emeasure || {:.4f} S-measure || {:.4f} AP || {:.4f} AUC || {:.4f} Dice || {:.4f} mIoU.'.format(
            time.time() - start_time, self.dataset, self.method, mae, max_f,
            mean_f, max_e, mean_e, s, avg_p, auc, dice, miou)

    def Eval_dice_mIoU(self):
        print('eval[Dice and mIoU]:{} dataset with {} method.'.format(self.dataset, self.method))
        avg_dice, avg_miou, img_num = 0.0, 0.0, 0.0
        ...
        with torch.no_grad():
            ...
            for pred, gt in self.loader:
                ...
                dice_score = self.dice(pred, gt)
                miou_score = self.mIoU(pred, gt)
            ...
            return avg_dice.item(), avg_miou.item()
